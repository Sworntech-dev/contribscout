...
export function OpportunityCard({
  ...
  return (
    <article className="rounded-md border border-white/10 bg-panel/75 p-5 transition hover:border-mint/40">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-sm capitalize text-skyglass">{opportunity.category}</p>
          <h3 className="mt-1 text-2xl font-black text-white">{opportunity.name}</h3>
          <p className="mt-2 text-sm leading-6 text-slate-400">{opportunity.description}</p>
        </div>
      ...
      <div className="mt-5 grid gap-4 border-t border-white/10 pt-5 sm:grid-cols-2">
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-white">{opportunity.suggestedAction}</p>
        </div>
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Reason</p>
          <p className="mt-2 text-sm leading-6 text-slate-300">{opportunity.scoreReason}</p>
        </div>
      </div>
      ...
      {isSampleFallback || !opportunity.url ? (
        <span className="mt-5 inline-flex rounded-md border border-white/10 bg-white/[0.03] px-3 py-2 text-sm font-semibold text-slate-400">
          Sample project
        </span>
      ) : (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="mt-5 inline-flex rounded-md border border-white/10 bg-white/5 px-3 py-2 text-sm font-semibold text-white transition hover:border-mint/50 hover:bg-mint/10"
        >
          Open repository
        </a>
      )}
